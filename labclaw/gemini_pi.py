"""Gemini principal investigator orchestration for LabClaw."""
...
PI_PROMPT_VERSION = "gemini-pi-v1"
PI_SCHEMA_VERSION = 1
DEFAULT_PI_MODEL = "gemini-2.5-pro"

SYSTEM_PROMPT = """You are LabClaw's Gemini principal investigator.
...
class GeminiPI:
    ...
    def decide(
        ...
        *,
        ...
        return decision_from_payload(load_json_object(raw))
...
class GeminiAPIClient:
    ...
    def __init__(self, *, api_key: str | None = None) -> None:
        ...
        try:
            ...
            raise RuntimeError("Install `google-genai` to use the live Gemini PI client.") from exc
...
def load_json_object(raw: Mapping[str, Any] | str | None) -> dict[str, Any]:
    if raw is None:
        raise ValueError("Gemini PI returned no decision content.")
    ...
    try:
        ...
        raise ValueError("Gemini PI returned malformed JSON.") from exc
    if not isinstance(parsed, dict):
        raise ValueError("Gemini PI decision JSON must be an object.")
    return parsed
...
def require_keys(payload: Mapping[str, Any], keys: Sequence[str], context: str) -> None:
    ...
    if missing:
        raise ValueError(f"Gemini PI {context} missing required field(s): {', '.join(missing)}")
